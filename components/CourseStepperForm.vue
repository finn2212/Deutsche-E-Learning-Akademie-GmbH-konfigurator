        ...
        <div class="overflow-y-auto max-h-[70vh]">
          <Stepper :steps="steps" :modelValue="currentStep" @update:modelValue="updateStep">
            <template #default="{ currentStep }">
              ...
              <Step2Component v-if="currentStep === 1" :course="course" @prevStep="prevStep" @nextStep="nextStep" @close="closeForm" />
              <Step3Component v-if="currentStep === 2" :course="course" @prevStep="prevStep" @close="closeForm" @add-course="addCourse" @course-saved="courseSaved" />
            </template>
          </Stepper>
        </div>
      </div>
